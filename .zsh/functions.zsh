...
if (( $+commands[singularity] )) && [[ ! $IS_SINGULARITY -eq 1 ]]; then
  function centos7()
  {
    ...
    if [[ -n ${SINGULARITY_CENTOS7+x} ]]; then
      if [[ $IS_NAF -eq 1 ]]; then
        singularity exec --contain --pwd "$(pwd)" --bind /afs:/afs --bind /cvmfs:/cvmfs --bind /data:/data --bind /nfs:/nfs --bind /pnfs:/pnfs --bind /var/tmp:/var/tmp --bind /tmp:/tmp "$SINGULARITY_CENTOS7" /bin/zsh -l
      else
